...
const SheetOverlay = React.forwardRef(({ className, ...props }, ref) => (
  <SheetPrimitive.Overlay
    className={cn(
      "fixed inset-0 z-50 bg-black/80 data-[state=open]:animate-fade data-[state=closed]:animate-fade",
      className
    )}
    {...props}
    ref={ref}
...
const SheetContent = React.forwardRef(
  ({ side = "right", className, children, ...props }, ref) => (
    <SheetPortal>
      ...
      >
        {children}
        <SheetPrimitive.Close className="absolute top-8 right-8 outline-none transition-opacity">
